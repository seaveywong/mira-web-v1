...
function renderNodeStatus(s) {
  if (s.nodeId) {
    ...
    let html = `已激活执行器：<b>${s.nodeName || s.nodeId}</b><br>`;
    html += `最近状态：${last.message || "等待心跳 / 等待任务"}<br>`;
    if (last.updatedAt) html += `更新时间：${last.updatedAt}<br>`;
    if (s.tokenExpiresAt) html += `本地 Token 到期：${new Date(s.tokenExpiresAt).toLocaleString()}<br>`;
    if (node.fb_user_name) html += `Meta 用户：${node.fb_user_name}<br>`;
    if (node.account_count !== undefined) html += `<span class="pill">${node.account_count || 0} 个账户可见</span>`;
    if (node.has_ads_management) html += `<span class="pill">ads_management</span>`;
    if (node.has_ads_read) html += `<span class="pill">ads_read</span>`;
    if (node.last_error) html += `<br>说明：${node.last_error}`;
    setStatus(html, last.ok !== false && node.status !== "token_error");
    return;
  }
...
$("heartbeatBtn").addEventListener("click", () => withButton($("heartbeatBtn"), "验证中...", async () => {
  await saveSettings();
  const data = await send({ type: "heartbeat" });
  const node = data.node || data;
  let html = `验证上报成功：${node.status || "online"}<br>`;
  html += `Meta 用户：${node.fb_user_name || "--"}<br>`;
  html += `可见账户：${node.account_count || 0} 个<br>`;
