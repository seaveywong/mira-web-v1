...
from services.token_manager import get_exec_token, ACTION_CREATE
...
class AutoPilotEngine:
    ...
    def _fb_post(self, path: str, token: str, payload: dict) -> dict:
        ...
        if "error" in data:
            ...
            logger.error(f"[AutoPilot] FB API Error on {path}: {data['error']} | payload={debug_payload}")
            raise Exception(f"FB API Error: {data['error'].get('message', str(data['error']))}")
        return data
        ...
        try:
            ...
            if _camp_tw_page:
                ...
                if _cp_row:
                    ...
                    # 优先使用 tw_certified_pages 中存储的 verified_identity_id
                    if _cp_row["verified_identity_id"]:
                        tw_verified_id = str(_cp_row["verified_identity_id"]).strip()
                        logger.info(f"[AutoPilot] 台湾认证身份（tw_certified_pages）: verified_id={tw_verified_id}")
                    else:
                        logger.info(f"[AutoPilot] 台湾认证身份（tw_certified_pages）: beneficiary={beneficiary}, payer={payer}（无 verified_identity_id）")
            # 第2优先：账户绑定的认证身份（tw_advertiser_id → tw_advertisers 表，旧版兼容）
            if not tw_verified_id and _acc and _acc.get("tw_advertiser_id"):
            ...
            fb_campaign_id = self._create_campaign(
                act_id, campaign["name"], campaign["objective"], token
            )
            ...
            for group_idx, audience in enumerate(audience_groups):
                try:
                    adset_name = f"{campaign['name'][:25]} - AS{group_idx+1}"
                    # v4.0: 设备端版位覆盖
                    ...
                    for ad_idx in range(ad_count):
                        try:
                            ...
                            ad_name = f"[AP] {asset['id']}-AD{ad_idx+1}"
                        ...
                        try:
                            ...
                            for ad_idx in range(ad_count):
                                try:
                                    ...
                                    body = bodies[ad_idx]
                                    asset_code = asset.get("asset_code") or f"AST-{asset['id']:04d}"
                                    ad_name = f"[AP] {asset['id']}-AD{ad_idx+1}-FB"
                                    fb_ad_id = self._create_ad(
                                        act_id, fb_adset_id, ad_name,
                                        headline, body, page_id,
                                        fb_asset_ref, asset["file_type"], token,
            ...
            conn.execute(
                ...
                (total_adsets, total_ads,
                 datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"), campaign_id)
            )
            conn.commit()
            conn.close()

            self._update_progress(campaign_id, "done", f"完成！共创建 {total_adsets} 个 AdSet，{total_ads} 条广告")
            logger.info(
        ...
        try:
            ...
            if _row:
                ...
                if fb_type == "image":
                    ...
                    return {"type": "video", "video_id": fb_id}
    ...
    def _update_asset_fb_ref(self, asset_id: int, fb_id: str, fb_type: str, act_id: str = ""):
        ...
        if act_id:
            try:
                conn.execute(
                    ...
                    "VALUES (?, ?, ?, ?, ?)",
                    (asset_id, act_id, fb_id, fb_type, datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"))
                )
            except Exception as _e:
        ...
        conn.execute(
            ...
            (fb_id, fb_type, datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"), asset_id)
        )
        conn.commit()
        conn.close()
    ...
    def _create_adset(
        ...
        if placements and placements.get("publisher_platforms"):
            ...
            if placements.get("facebook_positions"):
                targeting["facebook_positions"] = placements["facebook_positions"]
        ...
        payload = {
            ...
            "targeting": targeting,
            "status": "ACTIVE",
            "start_time": datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S+0000"),
        }
        ...
        if opt_goal in ("OFFSITE_CONVERSIONS", "VALUE"):
            ...
            # 主页赞：需要 page_id 作为 promoted_object，且必须设置 destination_type=ON_PAGE
            # 文档：PAGE_LIKES → OUTCOME_ENGAGEMENT + destination_type: ON_PAGE
            if not page_id:
                raise ValueError(
                    f"【主页赞】目标需要配置主页 ID。"
                    f"请在账户详情中配置主页，或在铺广告高级设置中选择主页。"
    ...
    def _ai_gen_msg_template(self, body: str, headline: str, asset_info: dict = None) -> dict:
        """根据素材AI分析结果，调用DeepSeek生成Messenger欢迎消息模版（与广告内容强相关）"""
        ...
        prompt = (
            "你是一位Facebook广告专家，请根据以下广告内容，生成一个高度相关的Messenger欢迎消息模版。\n\n"
            f"广告标题：{headline}\n"
            f"广告文案：{body[:300]}\n"
    ...
    def _create_ad(
        ...
        # ── 如果 message_template/lead_form_id 是数字字符串，则视为模版 ID，自动从模版库获取 ──
        # 消息模版 ID：从 msg_templates 表获取 FB 格式
        _msg_template_resolved = message_template
        if message_template and message_template.strip().isdigit():
            try:
                from api.ad_templates import get_msg_template_fb_format
                _fb_fmt = get_msg_template_fb_format(int(message_template.strip()))
                if _fb_fmt:
                    _msg_template_resolved = _json.dumps(_fb_fmt, ensure_ascii=False)
                    logger.info(f"[AutoPilot] 使用消息模版 ID={message_template}")
            except Exception as _e:
                logger.warning(f"[AutoPilot] 消息模版获取失败: {_e}")
        message_template = _msg_template_resolved

        # 表单模版 ID：在目标主页上自动创建 Lead Form，获取 fb_form_id
        ...
        if lead_form_id and lead_form_id.strip().isdigit():
            try:
                ...
                _fb_form_id = create_lead_form_for_page(page_id, int(lead_form_id.strip()))
                if _fb_form_id:
                    _lead_form_resolved = _fb_form_id
                    logger.info(f"[AutoPilot] Lead Form 自动创建成功: form_id={_fb_form_id}")
                else:
                    logger.warning(f"[AutoPilot] Lead Form 自动创建失败（可能缺少 pages_manage_ads 权限），将使用 SIGN_UP 链接替代")
                    _lead_form_resolved = ""
            except Exception as _e:
                logger.warning(f"[AutoPilot] Lead Form 创建异常: {_e}")
                _lead_form_resolved = ""
        ...
        def _build_cta(link: str) -> dict:
            ...
            if is_page_likes_ad:
                ...
                return {"type": "LIKE_PAGE", "value": {"page_id": page_id}}
            elif is_msg_ad:
                ...
                return {"type": "SIGN_UP", "value": {"lead_gen_form_id": lead_form_id}} if lead_form_id else {"type": "SIGN_UP", "value": {"link": link}}
            elif landing_url:
        ...
        if is_msg_ad:
            _page_welcome = None
            # 优先使用用户自定义模版，否则用 AI 文案自动生成
            if message_template:
                try:
                    _tid = int(str(message_template).strip())
                    from api.ad_templates import get_msg_template_fb_format
                    ...
                    try:
                        _page_welcome = _json.loads(message_template) if str(message_template).strip().startswith("{") else None
                    except Exception:
                        pass
            
            if _page_welcome is None:
                # 无模版或模版无效：调用 DeepSeek AI 根据素材信息自动生成欢迎消息
    ...
    def _update_campaign_status(self, campaign_id: int, status: str, error_msg: str = None):
        ...
        if error_msg:
            conn.execute(
                ...
                (status, error_msg[:500], datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"), campaign_id)
            )
        else:
            conn.execute(
                "UPDATE auto_campaigns SET status=?, updated_at=? WHERE id=?",
                (status, datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"), campaign_id)
            )
        conn.commit()
        conn.close()
    ...
    def _update_progress(self, campaign_id: int, step: str, msg: str = ""):
        ...
        conn.execute(
            ...
            (step, msg[:300] if msg else "", datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"), campaign_id)
        )
        conn.commit()
        conn.close()
    ...
    def _update_campaign_field(self, campaign_id: int, field: str, value):
        ...
        conn.execute(
            ...
            (value, datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"), campaign_id)
        )
        conn.commit()
        conn.close()
    ...
    def _insert_campaign_ad(
        ...
        # 广告创建成功后，同步写入 kpi_configs（含 target_cpa）
        if fb_ad_id and status == "done":
            try:
                camp_row = conn.execute(
                    "SELECT objective, target_cpa FROM auto_campaigns WHERE id=?",
                    (campaign_id,)
                ).fetchone()
                if camp_row:
                    from services.kpi_resolver import _OBJECTIVE_RULES, get_kpi_label, infer_ad_type
                    objective = camp_row["objective"] or "OUTCOME_SALES"
                    target_cpa = camp_row["target_cpa"]
                    kpi_field, kpi_label = _OBJECTIVE_RULES.get(
                        objective, ("offsite_conversion.fb_pixel_purchase", "像素购买")
                    )
                    ad_type = infer_ad_type(objective, "", "")
                    conn.execute(
                        """INSERT INTO kpi_configs
                           (act_id, level, target_id, target_name, kpi_field, kpi_label,
                            target_cpa, source, enabled, objective, ad_type, updated_at)
                           VALUES (?,?,?,?,?,?,?,?,1,?,?,datetime('now','+8 hours'))
                           ON CONFLICT(act_id, level, target_id) DO UPDATE SET
                           kpi_field=excluded.kpi_field,
                           kpi_label=excluded.kpi_label,
                           target_cpa=CASE WHEN excluded.target_cpa IS NOT NULL THEN excluded.target_cpa ELSE kpi_configs.target_cpa END,
                           source=CASE WHEN kpi_configs.source='manual' THEN 'manual' ELSE excluded.source END,
                           objective=excluded.objective,
                           ad_type=excluded.ad_type,
                           updated_at=datetime('now','+8 hours')""",
                        (act_id, "ad", fb_ad_id, headline or fb_ad_id,
                         kpi_field, kpi_label, target_cpa, "rule",
                         objective, ad_type)
                    )
                    conn.commit()
                    logger.info(f"[AutoPilot] KPI预配置写入: ad={fb_ad_id} kpi={kpi_field} target_cpa={target_cpa}")
            except Exception as _kpi_err:
                logger.warning(f"[AutoPilot] KPI预配置写入失败（非致命）: {_kpi_err}")
