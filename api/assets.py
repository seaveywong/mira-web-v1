...
def _launch_landing_link_base(value: Optional[str]) -> str:
    ...
    # services.landing_link_resolver. Behaviour unchanged.
    return _shared_landing_link_base(value)


def _launch_domain_status_usable(raw_response, last_error: str = "") -> bool:
    # Delegates to the shared canonical implementation. accept_true_status
    # defaults to True, preserving this call-site's historic behaviour
    # (it treated the literal token "true" as a usable status, unlike
    # launch_engine's copy). Kept explicit to prevent the #7-style drift.
    return _shared_domain_status_usable(raw_response, last_error, accept_true_status=True)
...
def _managed_landing_launch_issue(conn, landing_url: str, label: str = "落地页链接") -> str:
    ...
    for row in rows:
        ...
        # public_url decision is delegated to the shared resolve_public_url
        # helper (services.landing_link_resolver). accept_true_status=True
        # preserves this call-site's historic behaviour. The granular error
        # messages are preserved by re-checking the custom_domain branch.
        usable_url = _shared_resolve_public_url(item, accept_true_status=True)
        if not usable_url:
            if custom_domain:
